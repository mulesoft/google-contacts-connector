...
public class DeleteContactPhotoByIdTestCases extends GoogleContactsTestParent {

    @Before
    public void setUp() throws Exception {
        initializeTestRunMessage("deleteContactPhotoById");
    ...
    @Test
    public void testDeleteContactPhotoById() {
        try {
            ...
            assertTrue(hasContactPhoto(contactEntry));

            upsertOnTestRunMessage("id", extractEntryId(contactEntry));

            runFlowAndGetPayload("delete-contact-photo-by-id");

            // Wait for the remote google object to be refreshed.
            Thread.sleep(5000);

            GoogleContactEntry returnedContactEntry = runFlowAndGetPayload("get-contact-by-id");

            // Wait for the remote google object to be refreshed.
            Thread.sleep(5000);

            assertTrue(returnedContactEntry != null);
            assertFalse(hasContactPhoto(returnedContactEntry));
        } catch (Exception e) {
